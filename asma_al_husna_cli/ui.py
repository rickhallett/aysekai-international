...
def prompt_user_intention() -> str:
    ...
    # Create prompt panel
    prompt_text = Text(
        "Speak your truth for today...\n", style=f"bold {COLORS['purple']}"
    )
    prompt_text.append(
        "Share your intention, your experience, your heart's whisper",
    ...
    intention = Prompt.ask(
        f"[{COLORS['purple']}]►[/] [italic]Enter your intention[/]", console=console
    )
...
def display_divine_name(name: DivineName) -> None:
    ...
    title_text.append(
        f"The {name.number}th Beautiful Name of Allah",
        style=f"italic {COLORS['light_purple']}",
    )
    ...
    if name.pronunciation or name.phonetics:
        ...
        if name.pronunciation:
            pron_text.append(
                f"Pronunciation: {name.pronunciation}\n",
                style=f"italic {COLORS['light_purple']}",
            )
        if name.phonetics:
