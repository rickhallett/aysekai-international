...
def run_randomness_demo(iterations: int = 999):
    ...
    table = Table(title="Selection Distribution Analysis", show_header=True, header_style="bold magenta")
    ...
    for num, count in most_common:
        ...
        console.print(f"  #{num}: {count} times ({deviation:+.1f}% from expected) - {name.name_arabic}")
    ...
    chi_square = sum((count - expected_freq) ** 2 / expected_freq for count in selections.values())
    chi_square += (99 - total_names_selected) * expected_freq  # Account for unselected names
...
if __name__ == "__main__":
    ...
    @app.command()
    def demo(
        iterations: int = typer.Option(999, "--iterations", "-i", help="Number of selections to perform")
    ):
        """Demonstrate the ultra-randomness of the selection algorithm"""
