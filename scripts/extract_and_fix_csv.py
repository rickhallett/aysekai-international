#!/usr/bin/env python3
"""
Extract clean data from the properly formatted CSV and create a Notion-ready version
"""
...
def extract_clean_data():
    """Extract and clean the data from the attached properly formatted CSV"""

    # This is the clean data structure from the attached file
    clean_data = [
        # Header
        [
        ...
        [
            "Al-Bāqī (الباقي)",
            "96",
            "The Everlasting One; The Eternal; The One who remains forever; The One whose existence has no beginning nor end; The Immutable",
            '📿 SHARĪ\'A (الشريعة) - The Law | Allah alone is eternal - everything else perishes. While creation has a beginning and end, He has always existed and will remain forever. This world is temporary, but He is permanent. All will return to dust except His Face. | 🚶 ṬARĪQA (الطريقة) - The Path | Work for what remains, not what perishes. Seek the eternal by investing in good deeds, charity, and knowledge that outlasts you. Practice detachment from the temporary. Recite "Yā Bāqī" to remember the impermanence of trials and the permanence of His mercy. | 💎 ḤAQĪQA (الحقيقة) - The Reality | Time itself is His creation - He transcends it entirely. There is no "before" or "after" with Allah. In deep meditation, touch the timeless present where past and future dissolve. The eternal exists in this very moment, always here, always now. | 🌟 MA\'RIFA (المعرفة) - Gnosis | The mystic realizes their own essence is a divine breath that returns to the Everlasting. By dying to the temporal self, one discovers the eternal soul. In union with Al-Bāqī, the lover tastes immortality while still in this mortal frame.',
            '"Whatever you have will end, but what Allah has is lasting"; "And there will remain the Face of your Lord, Owner of Majesty and Honor"',
            "An-Nahl 16:96; Ar-Rahman 55:27",
            'Basic: يا باقي (Yā Bāqī) - "O Everlasting One" | Extended: يا باقي أبقني على ذكرك (Yā Bāqī abqinī \'alā dhikrik) - "O Everlasting, keep me constant in Your remembrance" | Meditation: يا باقي بعد فناء كل شيء (Yā Bāqī ba\'da fanā\'i kulli shay\') - "O You who remain after all things perish" | Traditional use: For spiritual permanence and overcoming fear of death',
            "",
            '"Al-Bāqī" is pronounced as al-baa-qee. • The "al" is pronounced like the English word "al" • The "bā" is pronounced "baa" with a long \'a\' sound as in "father" • The "qī" is pronounced "qee" where: ◦ The "q" is a deeper guttural sound from the back of the throat (like a \'k\' but deeper) ◦ The "ee" is pronounced as in "see" • Emphasis is on the first syllable: AL-baa-qee',
        ],
        # Al-Khabīr (31)
        [
            "Al-Khabīr (الخبير)",
            "31",
            "The All-Aware One; The All-Knowing",
            "Al-Khabīr (الخبير) - The All-Aware | 📿 SHARĪ'A (الشريعة) - The Law | Allah knows all things in their finest details - the hidden and the manifest, the whispers of the heart, the secrets of creation. His knowledge encompasses the seen and unseen realms. | 🚶 ṬARĪQA (الطريقة) - The Path | Cultivate inner sincerity knowing He is aware of your true intentions. Practice examining your heart's hidden motives. Recite \"Yā Khabīr\" 812 times when seeking insight into hidden matters or self-knowledge. | 💎 ḤAQĪQA (الحقيقة) - The Reality | His awareness is the very consciousness by which you know yourself. The light of awareness in your heart is a ray of Al-Khabīr. He knows you better than you know yourself because He is the Knower within you. | 🌟 MA'RIFA (المعرفة) - Gnosis | Through this name, the mystic receives kashf (unveiling) - the hidden wisdom in all events becomes apparent. One sees with the divine eye of awareness, understanding the secret workings of destiny and divine wisdom.",
            '"And He is the All-Subtle, The All-Aware"',
            "Al-Mulk:14",
            'يا خبير أخبرني (Yā Khabīr akhbirnī) - "O All-Aware, inform me"',
            "https://www.youtube.com/watch?v=x9ZWMofCCVE",
            '"Al-Khabir" is pronounced as al-kha-beer. The "al" is pronounced like the English word "al". The "kh" is a guttural sound similar to the "ch" in the Scottish word "loch" or the German "Bach". The "a" in "kha" is pronounced like the "a" in "father". The "b" is pronounced as in English, and the "ee" in "beer" is pronounced like the "ee" in "see".',
        ],
        # Ar-Raqīb (43)
        [
            "Ar-Raqīb (الرقيب)",
            "43",
            "The Vigilant Supervisor; The Monitor",
            "Ar-Raqīb (الرقيب) - The Vigilant Supervisor | 📿 SHARĪ'A (الشريعة) - The Law | Allah watches over all creation, seeing every action and deed. Nothing escapes His sight - He is the Ultimate Witness who will bring all to account on the Day of Judgment. | 🚶 ṬARĪQA (الطريقة) - The Path | Practice muraqaba (vigilant contemplation) - develop consciousness of Allah's watchfulness in every moment. When tempted by sin, remember \"He sees me.\" Recite \"Yā Raqīb\" 312 times daily to cultivate divine awareness. | 💎 ḤAQĪQA (الحقيقة) - The Reality | His watchfulness is not external surveillance but intimate presence. He is closer than your jugular vein, witnessing from within your own consciousness. Divine vigilance is an act of love, not judgment. | 🌟 MA'RIFA (المعرفة) - Gnosis | The realized soul becomes a mirror of this attribute - developing spiritual sight that sees Allah's presence in all things. In this station, the separation between Watcher and watched dissolves into unity.",
            "He is with you wherever you are; and Allah sees the things you do",
            "Al Hadîd:4",
            'يا رقيب احفظني (Yā Raqīb iḥfaẓnī) - "O Vigilant One, protect me"',
            "",
            "",
        ],
        # Al-Matīn (54)
...
def create_full_csv(output_file):
    """Create complete CSV with all names from the properly formatted data"""
    print(f"Creating complete CSV: {output_file}")

    # Get the clean sample data
    sample_data = extract_clean_data()

    with open(output_file, "w", encoding="utf-8", newline="") as file:
        writer = csv.writer(file, quoting=csv.QUOTE_ALL)

        for row in sample_data:
            # Clean each cell to ensure no problematic line breaks
            cleaned_row = []
            for cell in row:
    ...
    print(f"✅ Created CSV with {len(sample_data)} rows")
    print(f"✅ This includes header + {len(sample_data)-1} name entries")
    print(f"✅ File saved to: {output_file}")

    # Show sample of Ta'wil content
    if len(sample_data) > 1:
        print("\n📝 Sample Ta'wil content (Al-Bāqī):")
        tawil_content = sample_data[1][3]  # Ta'wil is column 4 (index 3)
...
if __name__ == "__main__":
    output_file = "asma_al_husna_notion_ready.csv"
    create_full_csv(output_file)
    print(f"\n🎉 Success! Import {output_file} into Notion")
    print(f"💡 This sample contains 5 carefully formatted entries.")
    print(
        f"💡 The Ta'wil column now preserves all four spiritual levels with | separators."
    )
    print(f"💡 Each entry maintains the complete spiritual explanations.")
