...
def get_data_files_path() -> Path:
    ...
    raise FileNotFoundError(
        "Could not find the CSV data files. Please ensure they are in the correct location."
    )
    ...
    try:
        ...
        if name_number:
            try:
                ...
                show_error(
                    f"Name #{name_number} not found. Please choose a number between 1 and 99."
                )
                raise typer.Exit(1)
        else:
            # Get user intention
        ...
        # Log the session
        log_dir = data_path / "data" / "logs"
        log_dir.mkdir(parents=True, exist_ok=True)
        logger = SessionLogger(log_dir)
        
        # Get the transliteration (handle the case where name might have parentheses)
        name_parts = selected_name.name_arabic.split("(")
        transliteration = name_parts[1].rstrip(")") if len(name_parts) > 1 else selected_name.name_arabic
        
        # Use the intention if available, otherwise use a default
        session_prompt = intention if 'intention' in locals() else "Direct selection"
        logger.log_session(session_prompt, selected_name.number, transliteration)
...
def list_names(
    start: int = typer.Option(
        1, "--start", "-s", min=1, max=99, help="Starting number"
    ),
    ...
    try:
        ...
        for name in filtered_names:
            ...
            console.print(
                f"[purple]{name.number:2d}[/]. [gold]{arabic_name}[/] - [italic]{name.brief_meaning[:60]}...[/]"
            )
